 ...
 */
const isExistsUsername = async (username, conn) => {
 ...
 */
const insertUser = async (username, password, conn) => {
    try{
        const sql = `INSERT INTO users (username, password, register_date, account) VALUES (?, ?, ?, ?)`;
        const param = [username, password, new Date(), 0];
 ...
 * @returns userData
 */
const getUser = async (userId, conn) => {
    try{
